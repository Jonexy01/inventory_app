...
class _HomeState extends State<Home> {
  ...
  Widget build(BuildContext context) {
    return StreamProvider<List<MyUserData>?>.value(
      value: DatabaseService().currentUserData,
      initialData: null,
      child: Scaffold(
          appBar: AppBar(
            // Here we take the value from the MyHomePage object that was created by
            // the App.build method, and use it to set our appbar title.
            title: MyTitleText(widget: this.widget),
            actions: <Widget>[
              IconButton(
                onPressed: () async {
...
class MyTitleText extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final userDocument = Provider.of<MyUserData>(context);
    String? titleText = userDocument.name;

    return Text(widget.title! + ' ' + titleText!);
  }
}
